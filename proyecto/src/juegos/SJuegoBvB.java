...
public class SJuegoBvB implements Serializable{
  ...
  //constructor
  public SJuegoBvB(String name1, String name2){

    jugador1 = new JugadorCpu(name1);
  ...
  //metodos
// Método para escribir fichas en un archivo binario
  private void escribirFichasEnArchivo(List<FichaDomino> fichas, String nombreArchivo) {
    ...
    // Método para leer fichas desde un archivo binario
    private List<FichaDomino> leerFichasDesdeArchivo(String nombreArchivo) {
      List<FichaDomino> fichas = new ArrayList<>();
  
      try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream(nombreArchivo))) {
          fichas = (List<FichaDomino>) ois.readObject();
  ...
  public void repartirFichas() {
    ...
    // Serializar las fichas para el jugador1 antes de repartirlas
    List<FichaDomino> fichasJugador1 = new ArrayList<>();
    for (int i = 0; i < 7; i++) {
    ...
    // Guardar las fichas serializadas
    escribirFichasEnArchivo(fichasJugador1, "fichasjugador1.cer");

    // Deserializar las fichas para el jugador1
    fichasJugador1 = leerFichasDesdeArchivo("fichasjugador1.cer");
    jugador1.recibirFicha( fichasJugador1);/////////

    // Repartir fichas aleatorias al jugador2
    for (int i = 0; i < 7; i++) {
        int maxF = fichasEnStack.size();
  ...
  private void guardarGanadorEnArchivo(String nombreGanador) {
    try {
        // Crear un objeto FileWriter sin especificar la ruta completa (se guardará en la ruta del programa por defecto)
        FileWriter fileWriter = new FileWriter("SJuegoBvB.txt", true);
...
public void repartirFichasS() {
    ...
    // Repartir fichas aleatorias al jugador1
    for (int i = 0; i < 7; i++) {
        int maxF = fichasEnStack.size();
        int m = rd.nextInt(maxF);
        FichaDomino ficha1 = fichasEnStack.get(m);
        jugador1.recibirFicha(ficha1);
        fichasEnStack.remove(m);
    }
}
  ...
  public void jugar(){
    ...
    while(this.sigueJuego()){
      ...
      */
      ...
      if(jugador1.puedeTirar){
        System.out.println("\nTermino el tiro de:" + jugador1 + "\n");
      }
      System.out.println("=================================");
    }
    this.printFichasEnMesa();
